...
- [ ] T006 Extend `VideoParams` in `MoneyPrinterTurbo/app/models/schema.py`: (a) add `"ugc_avatar"` to the `Mode` Literal type; (b) add `speaker_reference_path: Optional[str] = None`. Keep all existing fields untouched. Add inline comment referencing spec 018.
- [ ] T007 Create `MoneyPrinterTurbo/app/services/lip_sync.py` with the public surface declared in [contracts/l3-payload.md](./contracts/l3-payload.md): `detect_face(video_path)`, `extend_reference_to_duration(ref_path, target_seconds, output_path)`, `run(reference_path, audio_path, output_path, face_bbox=None)`. Module-level engine selection via `LIP_SYNC_ENGINE` env var (read once at import).
- [ ] T008 [P] Implement `detect_face()` in `MoneyPrinterTurbo/app/services/lip_sync.py` using `mediapipe.tasks` face-detection task. Sample 30 frames evenly across the video; pick the largest centred face with confidence ≥ 0.7; return `{x, y, w, h, confidence, frames_scanned, max_confidence}`.
- [ ] T009 [P] Implement `extend_reference_to_duration()` in `MoneyPrinterTurbo/app/services/lip_sync.py` using FFmpeg's ping-pong filter chain (`-filter_complex "[0:v]reverse[r];[0:v][r]concat=n=2:v=1"`) repeated until duration ≥ target, then `-t` trim to exact target. Per [research.md R5](./research.md#r5--loop-seam-smoothing-technique-fr-015).
- [ ] T010 [P] Implement device auto-detection helper in `MoneyPrinterTurbo/app/services/lip_sync.py`: `_pick_device()` returns `"cuda"` if `torch.cuda.is_available()` else `"mps"` if `torch.backends.mps.is_available()` else `"cpu"`. Honour `MUSETALK_DEVICE` env override. Log the chosen backend.
- [ ] T011 Implement MuseTalk-engine path in `run()` in `MoneyPrinterTurbo/app/services/lip_sync.py`: lazy-load the model on first call (cache module-level), run inference, write 9:16 output MP4. Use the device chosen by T010. **Aspect-ratio handling (F8)**: for any source aspect ratio (4:3, 16:9, 9:16), compute a 9:16-tight crop centered on the detected `face_bbox` from T008 — do NOT pass the full source frame to MuseTalk. The crop region anchors at the face center and extends ±9:16 of the face height; pad with edge replication if the face is closer to a frame edge than the half-crop dimension allows.
- [ ] T012 Implement mock-engine path in `run()` in `MoneyPrinterTurbo/app/services/lip_sync.py`: when `LIP_SYNC_ENGINE=mock`, copy the input reference verbatim to output_path (no inference). For dev/CI on hosts without GPU/MPS.
- [ ] T013 Create `MoneyPrinterTurbo/app/services/modes/ugc_avatar.py` per the contract in [contracts/l3-payload.md](./contracts/l3-payload.md#mode-registry-entry): `name="ugc_avatar"`, `default_aspect_ratio=portrait`, duration band 15-300s, `target_resolution=(1080,1920)`, `subtitle_band_y_pct=0.80`, `segment_count_range=None`, `script_template="HOOK_BODY_CTA"`, plus `generate_script` (delegates to `llm.generate_marketing_script` with voice-locale fallback), `generate_terms` (returns `[]`), `select_visuals_strategy` (returns `"user_uploaded"`).
- [ ] T014 Register the new mode in `MoneyPrinterTurbo/app/services/modes/__init__.py` per the existing registration pattern (alongside short, faceless, long).
- [ ] T015 Extend `MoneyPrinterTurbo/app/services/task.py` to dispatch `mode="ugc_avatar"`: insert lip-sync step between TTS audio synthesis (existing) and subtitle burn-in (existing). Call `lip_sync.extend_reference_to_duration()` if audio duration > reference duration; then `lip_sync.run()` to produce `lipsync.mp4`; then existing video-assembly path consumes `lipsync.mp4` instead of stock-clip concat.
- [ ] T016 Create selfie upload endpoint in `MoneyPrinterTurbo/app/controllers/v1/uploads.py` (new file or extend existing): `POST /api/v1/uploads/selfie` per [contracts/selfie-upload.md](./contracts/selfie-upload.md). Multipart parse → ffprobe duration/format → MediaPipe face-detect → reject early on validation failures (5-60s duration band per FR-001) → save to `storage/uploads/<tenant>/avatars/slot{1,2,3}/<uuid>.mp4` with mtime-based eviction.
- [ ] T017 Wire the uploads router into `MoneyPrinterTurbo/app/asgi.py` (or wherever existing controllers are registered). Verify `/api/v1/uploads/selfie` is reachable on the running L3.
- [ ] T018 Add `MoneyPrinterTurbo/app/utils/utils.py` helper `tenant_avatar_dir(tenant_id, slot)` that returns the absolute path `storage/uploads/<tenant>/avatars/slot{N}/`. Match the existing `task_dir` helper's signature/style.
- [ ] T019 [P] L1 scaffold: create `visualai-frontend/src/app/modes/ugc-avatar/page.tsx` with the same Sidebar + 4-step layout shape as `src/app/modes/short-video/page.tsx`. Empty placeholder steps; clicking "UGC Avatar" card on the dashboard routes here.
- [ ] T020 [P] L2 scaffold: create `visualai-orchestration/app/routes/uploads.py` with a placeholder `POST /api/v1/avatars/upload` that simply 501-NotImplemented for now. Wire into `visualai-orchestration/app/main.py`.
...
- [ ] T032 [P] [US1] Implement L3 list-recent endpoint at `MoneyPrinterTurbo/app/controllers/v1/uploads.py` `GET /api/v1/uploads/selfie/recent`: scan `storage/uploads/<tenant>/avatars/slot{1,2,3}/` for occupied slots, read each `<uuid>.meta.json`, return up to 3 newest as JSON array shaped per [contracts/selfie-upload.md](./contracts/selfie-upload.md#response--success). Empty array when no slots occupied.
